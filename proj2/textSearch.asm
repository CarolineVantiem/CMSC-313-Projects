...
init:                                                   ; Regsiter initializations 
	mov	rdx, record				; Place record into register
	mov	r12, [count]				; Char counter for user input
	mov	r13, [count]				; Char counter for record
	mov	r14, [count]				; Char placeholder for record / clear func.
	dec	r12					; Get rid of null char at end 
	dec	r13
	dec	r14
	mov	r15, 0					; Counter to keep track of position in record

loop:							; Main loop to traverse substring and record
	mov	al, [rsi]				; Move rsi index to al
	mov	cl, [rdx]
	cmp	al, cl					; Compare the given chars
	je	next					; If chars are =
	cmp	cl, 0					;
	je	nope					; Didn't find string in text
	inc	rdx
	inc	r15
	jmp	loop					; Recursive call to loop until end of text

clear:							; Didn't find string in middle of record
	sub	r13, r12				; Get position where left off
	sub	rdx, r13
	mov	r13, r14				; Move placeholder
	mov	rsi, buf				; Place substring back into rsi
	mov	r12, [count]				; Restart count
	dec 	r12
	inc	rdx					; Get next index of text
	inc	r15
	jmp	loop					; Loop through text

compare:						; Compare given chars
	mov	al, [rsi]
	mov	cl, [rdx]
	cmp	al, cl
	je	next					; If given chars are =
	jmp	clear					; String was not found in text

next:							; Gets next char in substring
	inc	rsi
	dec	r12
	cmp	r12, 0					; If found string in text
	je	found
	inc	rdx
	jmp	compare					; Keep comparing if not end of string
